...
The app shell is now generated by `elcarax_ui` instead of hardcoded app-level render primitives. The default console proof remains:

```bash
cargo run -p elcarax_app
```

It prints UI node, layout, primitive, text primitive, and dirty flag counts. The native-shell command remains the manual desktop smoke test:
